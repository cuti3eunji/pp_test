...
function makeid(){
	...
	$(function() {
		...
		//초기값을 오늘 날짜로 설정
		$('#bar_exp').datepicker('setDate', 'today'); //(-1D:하루전, -1M:한달전, -1Y:일년전), (+1D:하루후, -1M:한달후, -1Y:일년후)     
		...
		if (gtincode.length > 0) {
			$('select[name=ma_code] option[value=' + gtincode + ']').attr(
					'selected', true);
			//alert(gtincode);
		...
		$("#gen")
				.click(
						function() {
							...
							//상품식별코드 발리데이션(01)
							var gs1 = /^[0-9]{13,14}$/;
							var gs1code = $('select[name=ma_code]').val();
							...
							var lineNumIn = $('#bar_lot').val().trim();
							...
							var serialNumIn = $('#bar_serial').val().trim();
			...
			<form class="form-inline" id="generateForm" method="post">
				<div class="form-group">
					<p class="dotted" id="genBarcodeP">
						...
						<select class="form-control"id="ma_code" name="ma_code">
					...
					<c:forEach items="${masterList }" var="master">
						...
						<c:if test="${!empty zz }">
							<input id="gtinSpan" class="form-control" type="text"
								readonly="readonly" value="${fn:substring(zz.gs1code,2,18) }">
						</c:if>
					...
					<p class="dotted" id="genBarcodeP">
						...
						<c:if test="${!empty zz }">
							<input class="form-control" id="" name="bar_exp"	placeholder="YYMMDD" readonly="readonly"	value="${fn:substring(zz.expire,2,8) }">
						</c:if>
						<c:if test="${empty zz }">
							<input class="form-control" id="bar_exp" name="bar_exp"	placeholder="YYMMDD" readonly="readonly">
						</c:if>
					</p>
					...
					<p class="dotted" id="genBarcodeP">
						...
						<c:if test="${!empty zz }">
							<input class="form-control" id="bar_lot" name="bar_lot"	placeholder="5자리입력 " value="${zz.lNum }"  >
						</c:if>
						<c:if test="${empty zz }">
							<input class="form-control" id="bar_lot" name="bar_lot"	placeholder="5자리입력"  maxlength = "5">
								<span hidden="" style="color: red;">5자리이하로 입력해주세요</span>
						</c:if>
					</p>
					<p class="dotted" id="genBarcodeP">
						<label for="id">제품일련번호(21)</label>
						<c:if test="${!empty zz }">
							<input class="form-control" id="bar_serial" name="bar_serial"	placeholder="20자리이하입력" value="${zz.sNum }">
						</c:if>
						<c:if test="${empty zz }">
							<input class="form-control" id="bar_serial" name="bar_serial"	placeholder="20자리이하입력"  maxlength = "10">
						</c:if>

					</p>
					<p class="dotted" id="genBarcodeP">
						<label for="id">당신만의 코드</label>
						<c:if test="${!empty zz }">
							<input class="form-control" id="bar_code" name="bar_code"	placeholder="지시코드" value="">
						</c:if>
						<c:if test="${empty zz }">
							<input class="form-control" id="bar_code" name="bar_code"	placeholder="지시코드" value="${ran }"  readonly="readonly" >
						</c:if>
					...
					<c:if test="${!empty zz }">
						<p class="dotted" id="genBarcodeP">
							<label for="id">처음화면으로</label>
							 <a	href="${pageContext.request.contextPath }/barcodeMaseter"	style="color: red;">처음화면으로</a>
						</p>
					</c:if>
				</div>
			</form>
			...
			<c:if test="${empty zz }">
 			 ...
 			 <img alt="바코드" src="/img/barcode.gif"> 
				...
				<p class="dotted">
					...
					<a href="#"		onclick="printArea()" id="gogoprint">생성된 바코드 인쇄하려면 여기를 누르세요</a>
				</p>
			</c:if>
		</div>
		...
		<c:if test="${empty zz }">
			<div class="container" id="divv2">
				<button type="button" class="btn btn-info btn-lg"
					data-toggle="modal" data-target="#myModal" id="bbb">바코드 스캔</button>
			</div>
		</c:if>
			...
			<div class="modal-dialog">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<label>스캔해주세요</label> <br> <input type="text"
