			...
			<form class="form-inline" id="generateForm" method="post">
				<div class="form-group">
					<p class="dotted" id="genBarcodeP">
						<label for="id">상품식별코드(01)</label> <select class="form-control"
							id="gs1" name="gs1">
					...
					<p class="dotted" id="genBarcodeP">
						...
						<c:if test="${!empty zz }">
							<input class="form-control" id="" name="expire"
								placeholder="YYMMDD" readonly="readonly"
								value="${fn:substring(zz.expire,2,8) }">
						</c:if>
						<c:if test="${empty zz }">
					...
					<c:if test="${empty zz }">
						<p class="dotted" id="genBarcodeP">
							...
							<button id="goLabel" type="button" class="btn btn-warning">라벨로
								인쇄하기</button>
						</p>
					</c:if>
					<c:if test="${!empty zz }">
						<p class="dotted" id="genBarcodeP">
							<label for="id">처음화면으로</label> <a
								href="${pageContext.request.contextPath }/home"
								style="color: red;">처음화면으로</a>
						</p>
					</c:if>
				</div>
			</form>
